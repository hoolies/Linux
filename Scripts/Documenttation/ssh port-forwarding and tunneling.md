...
**macOS — essentially identical to Linux.** macOS is a Unix (its network stack
...
**Windows — same protocol, different internals.** Modern Windows 10/11 ships
...
- Sockets are **Winsock** (`ws2_32.dll`) objects, not Unix file descriptors.
  "Everything is a file" does **not** apply — a socket is its own kind of
...
`ProxyCommand` tells ssh: *"don't open a TCP socket to the target yourself —
instead run this command, and use its **stdin/stdout** as the byte pipe to the
...
### Firefox (best — true per-application proxy)
...
background giving you a perfectly good SSH session with a **broken proxy** —
the worst kind of failure because everything *looks* fine. Always pair it with
...
**HTTPS/TLS payloads stay private** between your browser and the destination —
the exit host only relays encrypted bytes (SOCKS sits at the TCP layer, below
...
- **Proxy DNS** = send the hostname (not IP) so the **exit host** resolves it —
  essential for internal / split-horizon names.
...
- **Trust** = intermediate hops see only ciphertext; the **exit host** sees all
  destinations + any plaintext. Never agent-forward (`-A`) to untrusted hops —
  `ProxyJump` keeps your key on the laptop.
