...
description: >-
  Write and edit POSIX/bash shell scripts with a coreutils-style usage()
  function, shebang via /usr/bin/env, function-first structure, and a
  shellcheck pass that must be clean. Use when creating or changing .sh
  scripts, bash scripts, shell utilities, or zsh plugins; when the user
  asks for a script, usage/help text, or shellcheck; or when editing
  path_checker, path_cleaner, or other Bourne_Again shell tools.
---
